...
def provision_cluster(cluster_data):
    ...
    print("pwd", pwd)
    time.sleep(10)
    cmds = [f"terraform init", f"terraform apply -var-file ../../cluster-templates/{cluster_data['cloud_provider']}_dev_cluster.json"]
    for cmd in cmds:
...
def deprovision_cluster(cluster_data):
    print("pwd", pwd)
    cmds = [f"terraform destroy -var-file {cluster_data}"]
    for cmd in cmds:
